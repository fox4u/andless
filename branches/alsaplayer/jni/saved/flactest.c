...
static FLAC__bool write_little_endian_int24(FILE *f, FLAC__uint32 x)
{
	return
		fputc(x, f) != EOF &&
		fputc(x >> 8, f) != EOF &&
...
FLAC__StreamDecoderWriteStatus write_callback(const FLAC__StreamDecoder *decoder, const FLAC__Frame *frame, const FLAC__int32 * const buffer[], void *client_data)
{
	...
	for(i = 0; i < frame->header.blocksize; i++) {
		...
		write_little_endian_int24(f, (FLAC__int16)buffer[0][i]);   /* left channel */
		write_little_endian_int24(f, (FLAC__int16)buffer[1][i]);     /* right channel */
...
//		fprintf(stderr, "total samples  : %" PRIu64 "\n", total_samples);
		fprintf(stderr, "blocksize      : %u/%u\n", metadata->data.stream_info.min_blocksize,
				 metadata->data.stream_info.max_blocksize);
		fprintf(stderr, "framesize      : %u/%u\n", metadata->data.stream_info.min_framesize,
