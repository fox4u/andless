...
/* Expects null-terminated list of control names with their values as strings.
   All values of multi-valued controls are set to the same value for now.		
   Returns the number of controls found and set successfully. */

int set_mixer_controls(int card, char **names, char **values) 
{
    int i,j,k,m,fd,ret = 0;
    char tmp[256];		
    struct snd_ctl_elem_list elist;
    struct snd_ctl_elem_id *eid = 0;
	...
	/* get number of controls, allocate that much memory for their ids, and get those ids */
	...
	for(i = 0; names[i]; i++) {
		...
		if(strcmp(ei.id.name, names[i]) != 0) continue; /* not ours */
		memset(&ev, 0, sizeof(ev));
		ev.id.numid = ei.id.numid;
		switch(ei.type) {
		    case SNDRV_CTL_ELEM_TYPE_BOOLEAN:
		    case SNDRV_CTL_ELEM_TYPE_INTEGER:
			for(j = 0; j < ei.count; j++) ev.value.integer.value[j] = atoi(values[i]);
			if(ioctl(fd, SNDRV_CTL_IOCTL_ELEM_WRITE, &ev) < 0)
			    log_err("failed to set value for %s\n", names[i]);
			else {
			    log_info("%s -> %s\n", names[i], values[i]);
			    ret++;
			}
			...
			for(j = 0; j < ei.value.enumerated.items; j++) {  /* find enum index j that refers to our value */
			    ei1.value.enumerated.item = j;
			    if(ioctl(fd, SNDRV_CTL_IOCTL_ELEM_INFO, &ei1) < 0) {
				log_err("cannot name[%d] of enum control %s\n", j, names[i]);
				break;
			    }  
			    if(strcmp(ei1.value.enumerated.name, values[i]) == 0) {  /* that's it! */
				for(m = 0; m < ei.count; m++) ev.value.enumerated.item[m] = j;
				if(ioctl(fd, SNDRV_CTL_IOCTL_ELEM_WRITE, &ev) < 0)
				    log_err("failed to set value for %s\n", names[i]);
				else {
			    	    log_info("%s -> %s\n", names[i], values[i]);
			...
			if(j == ei.value.enumerated.items) 
			    log_err("failed to find value %s among those permitted for %s\n", values[i], names[i]);
			break;
		    default:
			log_err("cannot handle control %s of type %d\n", names[i], ei.type);
			break;	
		}
		break;	/* to next control; assuming that the same control doesn't occur more than once */	
	    } 
	}
	close(fd);
	free(eid);
...
int main(int argc, char **argv) {
    char *names[] = { "MultiMedia1 EQ Enable", "RX2 Digital Volume", "SLIM_0_RX Format", 0 };
    char *values[] = { "0", "52",  "S24_LE", 0 };	
    int ret;
	ret = set_mixer_controls(0, names, values);
    return ret;	
}
